...
/**
 * This class implements a Gourmet Coffee System and creates a console interface
 * to process store orders.
 *
 * @author A Jiayi
 * @version 1.0.0
 * @see Catalog
...
public class GourmetCoffee {
	...
	/**
	 * Console I/O standard input/output stream objects.
	 */
	...
	private Catalog loadCatalog() {
		...
		catalog.addProduct(new Coffee("C001", "Colombia, Whole, 1 lb", 17.99, "Colombia", "Medium", "Rich and Hearty",
				"Rich", "Medium", "Full"));
		catalog.addProduct(new Coffee("C002", "Colombia, Ground, 1 lb", 18.75, "Colombia", "Medium", "Rich and Hearty",
				"Rich", "Medium", "Full"));
	...
	private void run() throws IOException {
		...
		while (choice != 0) {
			...
			if (choice == 1) {
				...
				addModifyOrderProduct();
			} else if (choice == 5) {
				removeOrderProduct();
			} else if (choice == 6) {
				registerCurrentOrder();
			} else if (choice == 7) {
				displaySales();
			} else if (choice == 8) {
	...
	public void displayCurrentOrder() {
		...
		if (size == 0) {
			...
			stdOut.println("Total: " + CURRENCY.format(currentOrder.getTotalCost()));
		}
	}

	/**
	 * Modifies the current order: if the specified product is not already part of
	 * the order, it is added; otherwise, the quantity of the specified product is
	 * updated.
	 */
	public void addModifyOrderProduct() throws IOException {
	 ...
	 */
	public void removeOrderProduct() throws IOException {
		...
		if (numOrders != 0) {
			...
			for (Iterator<Order> i = sales.getOrdersIterator(); i.hasNext();) {
				...
				for (Iterator<OrderItem> j = order.getItemsIterator(); j.hasNext();) {
					stdOut.println("   " + ((OrderItem) j.next()).toString());
				}
	 ...
	 *
	 */
	public void displayNumberOfOrders() throws IOException {
		...
		stdOut.println("Number of orders that contains the product " + product.getCode() + ": " + count);
	}

	/**
	 * Displays the total quantity of product that has been sold for each product in
	 * the catalog.
	 */
	public void displayTotalQuantityOfProducts() {
		...
		for (Iterator<Product> i = catalog.getProductsIterator(); i.hasNext();) {
			Product product = (Product) i.next();
			int quantity = 0;
			for (Iterator<Order> j = sales.getOrdersIterator(); j.hasNext();) {
	 ...
	 *
	 * @return reference to the <code>Product</code> object with the specified code
	 */
